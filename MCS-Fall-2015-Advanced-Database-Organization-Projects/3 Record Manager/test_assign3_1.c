...
#define ASSERT_EQUALS_RECORDS(_l,_r, schema, message)			\
		do {									\
			Record *_lR = _l;                                                   \
			Record *_rR = _r;                                                   \
			ASSERT_TRUE(memcmp(_lR->data,_rR->data,strlen(_rR->data)) == 0, message); \
			int i;								\
...
main (void) 
{
	...
	while(1){
		...
		case 1:
			...
			printf("Schema created and value if attributes are %s\t%s\t%s\n",result->attrNames[0],result->attrNames[1],result->attrNames[2]);
			break;
		case 2:
		...
		case 4:
			testRecords();
			break;
		case 5: testUpdateTable();
			break;
		case 6: testScans();
			break;
		case 7: testMultipleScans();
			break;
		case 8: testScansTwo();
			break;
		case 9: exit(1);
...
testCreateTableAndInsert (void)
{
	...
	RID *rids;
	Schema *schema;
	testName = "test running muliple scans";//"test creating a new table and inserting tuples";
	schema = testSchema();
