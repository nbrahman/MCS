...
char *serializeValue(Value *val)
{
  ...
  printf("The data in serialise value is %d\n",val->dt);
  switch(val->dt)
    {
    case DT_INT:
      APPEND(result,"%i",val->v.intV);
      printf("The value passed is %d\n",val->v.intV);
      printf("In serializeValue:case DT_INT\n");
      break;
    case DT_FLOAT:
      APPEND(result,"%f", val->v.floatV);
      printf("In serializeValue:case DT_FLOAT\n");
      break;
    case DT_STRING:
      APPEND(result,"%s", val->v.stringV);
      printf("In serializeValue:case DT_STRING\n");
      break;
    case DT_BOOL:
      APPEND_STRING(result, ((val->v.boolV) ? "true" : "false"));
      printf("In serializeValue:case DT_BOOL\n");
      break;
    }
