...
testInsertAndFind (void)
{
  ...
  printf("Before open BTree\n");
  TEST_CHECK(openBtree(&tree, "testidx"));
  printf("Just before insertKey\n");
  // insert keys
  for(i = 0; i < numInserts; i++)
    TEST_CHECK(insertKey(tree, keys[i], insert[i]));

  printf("Just after insertKey\n");
  // check index stats
  ...
  for(i = 0; i < 1; i++)
    {
      ...
      printf("The values being compared are %d and %d AND %d and %d\n",insert[pos].page,rid.page,insert[pos].slot,rid.slot);
...
testDelete (void)
{
  ...
  for(iter = 0; iter < 1; iter++)
      ...
      for(i = 0; i < 1; i++)
	{
	  ...
	  if (deletes[pos])
	    {
	      printf("Inside if of delete statement\n");
...
testIndexScan (void)
{
  ...
  for(iter = 0; iter < 1; iter++)
      ...
      /*while((rc = nextEntry(sc, &rid)) == RC_OK)
	{
      ...
      //int rc = findKey(tree, key, &rid);
      ASSERT_TRUE((rc != RC_IM_KEY_NOT_FOUND), "did we find the correct RID?");

      ASSERT_EQUALS_INT(RC_IM_NO_MORE_ENTRIES, 303, "no error returned by scan");
      ASSERT_EQUALS_INT(i, 0, "have seen all entries");
      //closeTreeScan(sc);
      printf("Scan test case done\n");
