...
RC createPageFile (char *fileName)
{
	...
	if (fPageFile != NULL)
	{
		...
		else
		{
			...
			/*sm_fd->FileType = (char *) malloc (strlen (strFileType)+1);
			strcpy (sm_fd->FileType, strFileType);
			
			sm_fd->FileMode = (char *) malloc (strlen (strFileMode)+1);
...
RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage)
{
	...
	if(pageNum>=0){
		...
		filePointer = fopen(fHandle->fileName,"ab"); // open the binary file in write mode
